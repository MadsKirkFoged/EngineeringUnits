...
[TestClass]
public class ElectricSurfaceChargeDensityTests
{
    ...
    public void ElectricSurfaceChargeDensityAutoTest()
    {
        ...
        foreach (ElectricSurfaceChargeDensityUnit EU in UnitTypebase.ListOf<ElectricSurfaceChargeDensityUnit>())
        {

            var Error = 1E-5;
            var RelError = 1E-5;

            IEnumerable<UnitsNet.Units.ElectricSurfaceChargeDensityUnit> UNList = UnitsNet.ElectricSurfaceChargeDensity.Units.Where(x => x.ToString() == EU.QuantityName);

            if (UNList.Count() == 1)
            {
                UnitsNet.Units.ElectricSurfaceChargeDensityUnit UN = UNList.Single();
