...
public class ApparentEnergyTests
{
    ...
    public void AutoTest()
    {
        ...
        foreach (ApparentEnergyUnit EU in UnitTypebase.ListOf<ApparentEnergyUnit>())
        {

            var Error = 1E-5;
            var RelError = 1E-5;

            IEnumerable<UnitsNet.Units.ApparentEnergyUnit> UNList = UnitsNet.ApparentEnergy.Units.Where(x => x.ToString() == EU.QuantityName);

            if (UNList.Count() == 1)
            {
                UnitsNet.Units.ApparentEnergyUnit UN = UNList.Single();
