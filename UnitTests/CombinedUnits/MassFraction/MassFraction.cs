...
public class MassFractionTest
{
    ...
    public void MassFractionAutoTest()
    {
        ...
        foreach (RatioUnit EU in UnitTypebase.ListOf<RatioUnit>())
        {
            ...
            IEnumerable<UnitsNet.Units.MassFractionUnit> UNList = UnitsNet.MassFraction.Units.Where(x => x.ToString() == EU.QuantityName);

            if (UNList.Count() == 1)
            {
                UnitsNet.Units.MassFractionUnit UN = UNList.Single();
