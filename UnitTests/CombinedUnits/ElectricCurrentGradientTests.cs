...
[TestClass]
public class ElectricCurrentGradientTests
{
    ...
    public void AutoTest()
    {
        ...
        foreach (ElectricCurrentGradientUnit EU in UnitTypebase.ListOf<ElectricCurrentGradientUnit>())
        {

            var Error = 1E-5;
            var RelError = 1E-5;

            IEnumerable<UnitsNet.Units.ElectricCurrentGradientUnit> UNList = UnitsNet.ElectricCurrentGradient.Units.Where(x => x.ToString() == EU.QuantityName);

            if (UNList.Count() == 1)
            {
                UnitsNet.Units.ElectricCurrentGradientUnit UN = UNList.Single();
