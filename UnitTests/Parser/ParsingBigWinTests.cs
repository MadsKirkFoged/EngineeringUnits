...
namespace UnitTests.Parsing
{
    ...
    public class ParsingBigWinTests
    {
        ...
        private static decimal ToJoules(UnknownUnit u)
        {
        ...
        {
            var ok = UnknownUnitParser.TryParse(input, out var u, Inv);
        ...
        {
            var ok = UnknownUnitParser.TryParse(input, out var u, Da);
        ...
        {
            Assert.IsTrue(UnknownUnitParser.TryParse("1 N*m", out var u1, Inv));
            Assert.IsTrue(UnknownUnitParser.TryParse("1 kg*m^2/s^2", out var u2, Inv));
        ...
        public void UnknownUnit_EnergyEquivalence_WattHour_Equals_3600_Joule()
        {
            ...
            Assert.IsTrue(UnknownUnitParser.TryParse("1 W*h", out var u1, Inv), "Expected parse OK for '1 W*h'");
            Assert.IsTrue(UnknownUnitParser.TryParse("1 W h", out var u2, Inv), "Expected parse OK for '1 W h'");
        ...
        public void UnitParser_Generic_LengthUnit_Tokens(string token)
        {
            ...
            var ok = UnitParserTUnit<LengthUnit>.TryParse(token, out var u);
        ...
        public void UnitParser_Generic_EnergyUnit_Tokens_Minimal(string token)
        {
            ...
            var ok = UnitParserTUnit<EnergyUnit>.TryParse(token, out var u);
            if (token == "EnergyUnit")
            {
        ...
        {
            var ok = UnknownUnitParser.TryParse(input, out var _, Inv);
        ...
        public void UnknownUnitParser_Parse_ShouldThrowFormatException_OnInvalid()
        {
            ...
            {
                _ = UnknownUnitParser.Parse("10 totallyNotAUnit", Inv);
            });
        }
        ...
        public void UnknownUnitParser_ManyCalls_ShouldBeStable()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                Assert.IsTrue(UnknownUnitParser.TryParse("1 kg m^2 / s^2", out var u, Inv));
