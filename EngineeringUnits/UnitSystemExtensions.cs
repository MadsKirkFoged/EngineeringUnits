...
public static class UnitSystemExtensions
{
    ...
    public static UnitSystem Sqrt(this UnitSystem local)
    {
        ...
        foreach (RawUnit item in local.ListOfUnits.Where(x => x.UnitType is not BaseunitType.CombinedUnit))
        {
            if (item.Count % 2 != 0)
    ...
    public static Fraction ConvertionFactor(this UnitSystem From, UnitSystem To)
    {
        //lock (FactorLock)
        {
            (int, int) Hashes = (From.GetHashCode(), To.GetHashCode());

            if (CacheFactor.TryGetValue(Hashes, out Fraction local))
                return local;

            Fraction test = To.SumConstant() / From.SumConstant();
            _ = CacheFactor.TryAdd(Hashes, test);
            return test;
        }
    }

    private static readonly ConcurrentDictionary<int, Fraction> SumConstantCache = new();
    public static Fraction SumConstant(this UnitSystem From)
    {
        ...
        Fraction test = From.ListOfUnits.Aggregate(Fraction.One, (x, y) => x * y.TotalConstant);
    ...
    public static UnitSystem ReduceUnits(this UnitSystem a)
    {
        ...
        IEnumerable<RawUnit> NewUnitList = a.ListOfUnits.ReduceUnits();
    ...
    public static UnitSystem ReduceUnitsHard(this UnitSystem a)
    {
        ...
        IEnumerable<IGrouping<BaseunitType, RawUnit>> test = a.ListOfUnits.GroupBy(x => x.UnitType);

        var NewUnitList = new List<RawUnit>();

        foreach (IGrouping<BaseunitType, RawUnit> GroupOfTypes in test)
        {
    ...
    public static UnitSystem Pow(this UnitTypebase a, int toPower) => a.Unit.Pow(toPower);
}
