...
public partial class PressureChangeRate : BaseUnit
{
    ...
    public PressureChangeRate(Fraction value, PressureChangeRateUnit selectedUnit) : base(value, selectedUnit.Unit) { }
    ...
    public override string? GetStandardSymbol(UnitSystem _unit, string? format = null) => GetStandardSymbol<PressureChangeRateUnit>(_unit, format);

    public static PressureChangeRate Parse(string? input, IFormatProvider? culture = null)
    {
         return QuantityParser.Parse<PressureChangeRate, PressureChangeRateUnit>(
         input,
         (v, u) => new PressureChangeRate(v, u),
         PressureChangeRateUnit.SI,
         culture);
    }
}
