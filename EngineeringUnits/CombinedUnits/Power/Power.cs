...
[UnitDimension(BaseunitType.time, -3, BaseunitType.length, 2, BaseunitType.mass, 1, BaseunitType.electricCurrent, 0, BaseunitType.temperature, 0, BaseunitType.amountOfSubstance, 0, BaseunitType.luminousIntensity, 0, BaseunitType.Cost, 0)]
public partial class Power : BaseUnit
{
    public Power() { }
    public Power(Fraction value, PowerUnit selectedUnit) : base(value, selectedUnit.Unit) { }
    ...
    public override string? GetStandardSymbol(UnitSystem _unit, string? format = null) => GetStandardSymbol<PowerUnit>(_unit, format);

    public static Power Parse(string? input, IFormatProvider? culture = null)
    {
         return QuantityParser.Parse<Power, PowerUnit>(
         input,
         (v, u) => new Power(v, u),
         PowerUnit.SI,
         culture);
