...
{
    public PowerDensity() { }
    public PowerDensity(Fraction value, PowerDensityUnit selectedUnit) : base(value, selectedUnit.Unit) { }
    ...
    public override string? GetStandardSymbol(UnitSystem _unit, string? format = null) => GetStandardSymbol<PowerDensityUnit>(_unit, format);

    public static PowerDensity Parse(string? input, IFormatProvider? culture = null)
    {
         return QuantityParser.Parse<PowerDensity, PowerDensityUnit>(
         input,
         (v, u) => new PowerDensity(v, u),
         PowerDensityUnit.SI,
         culture);
    }
}
