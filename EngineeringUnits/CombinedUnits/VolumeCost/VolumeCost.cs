...
[UnitDimension(BaseunitType.time, 0, BaseunitType.length, -3, BaseunitType.mass, 0, BaseunitType.electricCurrent, 0, BaseunitType.temperature, 0, BaseunitType.amountOfSubstance, 0, BaseunitType.luminousIntensity, 0, BaseunitType.Cost, 1)]
public partial class VolumeCost : BaseUnit
{
    public VolumeCost() { }
    public VolumeCost(Fraction value, VolumeCostUnit selectedUnit) : base(value, selectedUnit.Unit) { }
    ...
    public override string? GetStandardSymbol(UnitSystem _unit, string? format = null) => GetStandardSymbol<VolumeCostUnit>(_unit, format);

    public static VolumeCost Parse(string? input, IFormatProvider? culture = null)
    {
         return QuantityParser.Parse<VolumeCost, VolumeCostUnit>(
         input,
